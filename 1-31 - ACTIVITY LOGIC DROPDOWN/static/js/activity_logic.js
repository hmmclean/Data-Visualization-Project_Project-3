const ActivityMap = {
  ...
  populateDropdown: function (data, activityTypeDropdown, regionDropdown) {
    ...
    const uniqueActivityArray = Array.from(uniqueActivities).sort();
    ...
    activityDropdown.removeEventListener('change', ActivityMap.activityDropdownChangeHandler);

    // Attach a new event listener to handle changes in the activity dropdown
    activityDropdown.addEventListener('change', ActivityMap.activityDropdownChangeHandler);
  },

  activityDropdownChangeHandler: function () {
