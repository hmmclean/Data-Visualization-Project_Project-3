...
let geojsonFeatures; // Declare geojsonFeatures at a global level to make it accessible
...
// Function to update the map based on selected values
function updateMap(selectedPark) {
  ...
  // Filter GeoJSON features based on the selected park
  const selectedParkFeature = geojsonFeatures.find(function (feature) {
    return feature.properties.fullName === selectedPark;
  });

  // Create a GeoJSON layer with the selected park feature
  if (selectedParkFeature) {
    const selectedParkLayer = L.geoJSON(selectedParkFeature, {
      ...
      onEachFeature: function (feature, layer) {
        // Existing popup configuration...
      }
    });

    // Add the selected park layer to the map
    selectedParkLayer.addTo(myMap);

    // Zoom to the selected park's location
    myMap.setView(selectedParkLayer.getBounds().getCenter(), 12);
  }
}
